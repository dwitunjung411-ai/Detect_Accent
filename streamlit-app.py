...
# --- 1. REGISTRASI CLASS PROTOTYPICAL ---
...
# --- 2. DATA PREPROCESSING (PERBAIKAN LINE 69) ---
@st.cache_resource
def load_and_preprocess_data():
    # Pastikan path ke folder audio benar
    path = "dataset_audio" # Sesuaikan dengan folder audio Anda
    ...
    # Progress bar karena ekstraksi MFCC memakan waktu
    progress_bar = st.progress(0)
    for i, row in metadata.iterrows():
        file_name = str(row['file_name'])
        file_path = os.path.join(path, file_name) if os.path.isdir(path) else file_name
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_final, y_aksen, test_size=0.2, stratify=y_aksen)
...
def load_my_model(path):
    return tf.keras.models.load_model(path, custom_objects={"PrototypicalNetwork": PrototypicalNetwork})

pn_model = load_my_model("model_aksen.keras")
